# send_voice (provisional name)

Place an outbound voice call that speaks supplied text (Twilio TwiML).
...
Twilio Calls API with inline TwiML <Say> via stdlib urllib; credentials from ctx.auth_get('twilio').

Secrets live in the credential store (ctx.auth_get), never in this repo, never in logs, never in error messages.

## Honest limits

REAL Twilio API call — requires credentials + human approval. Twilio per-minute charges apply on your Twilio account. No credentials -> AuthMissing. Demo mode fakes the HTTP layer, clearly labeled.
