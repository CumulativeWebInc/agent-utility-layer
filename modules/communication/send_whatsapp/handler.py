"""send_whatsapp — Crew C / communication pillar.

Send a WhatsApp message via Twilio's WhatsApp API (stdlib urllib).
External send: requires ctx.approval_request(...) before dispatch.
No credential -> AuthMissing. Never fabricates a "sent" confirmation.
"""
import os as _os
import sys as _sys
...
TWILIO_SETUP_HINT = (
    ...
    '"whatsapp_from": "+15551234567"} (the Twilio WhatsApp-enabled sender).'
)
...
def _provider_send(cred, inputs):
    for field in ("account_sid", "auth_token", "whatsapp_from"):
    ...
    data = urllib.parse.urlencode({
        ...
        "From": "whatsapp:" + cred["whatsapp_from"],
        "Body": inputs["message"],
    }).encode("utf-8")
    ...
    try:
        ...
        raise ModuleError("twilio whatsapp request failed: %s: %s" % (type(exc).__name__, exc))
    if not body.get("sid"):
        raise ModuleError("twilio whatsapp send failed: %s" % body)
...
def execute(inputs, ctx):
    ...
    approval_id = ctx.approval_request(
        "Send WhatsApp message to %s (%d chars)" % (inputs["to"], len(inputs["message"])))
    cred = json_cred(ctx, "twilio", TWILIO_SETUP_HINT)
    result = _provider_send(cred, inputs)
    ctx.log("send_whatsapp", {"to": inputs["to"], "approval_id": approval_id,
                              "message_sid": result["message_sid"]})
    ctx.bill(PRICE_PER_EXEC_USD, "send_whatsapp execution")
